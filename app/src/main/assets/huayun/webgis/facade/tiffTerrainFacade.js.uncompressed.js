define("com/huayun/webgis/facade/tiffTerrainFacade", [
    ...
    function loadTifTerrain(url, success, error) {
        request.get(url, {handleAs: "arraybuffer"}).then(function (buffer) {
            ...
            for (i = 0; i < imageHeight; i++) {
                ...
                var count = 0;
                var posOffset = (i % tileHeight) * tileWidth * 2;
                for (var j = 0; j < colCount; j++) {
                    var tileIndex = row * colCount + j;
                    pos = tags.TileOffsets.value[tileIndex] + posOffset;
                    for (var k = 0; k < tileWidth; k++) {
                        var h = view.getInt16(pos, true);
                        if (h === noData) {
                            h = 0;
                        }
                        heights[count++] = h;
    ...
    exports.loadTifDat = function (url, success, error) {
        request.get(url, {handleAs: "arraybuffer"}).then(function (buffer) {
            var view = new DataView(buffer);
            var pos = 0;
            var tiles = [];
            for (var i = 0; i < 31 * 128; i++) {
                var tile = [];
                for (var j = 0; j < 27 * 128; j++) {
                    tile.push(view.getInt16(pos, true));
                    pos += 2;
                }
                tiles.push(tile);
            }
            var result = {
                ...
                imageWidth: 3415,
                imageHeight: 3882,
                reference: [0, 0, 0, 497038.9953648312, 3433999.3373779617, 0],
                resolutions: [30, 30, 0],
                tileWidth: 128,
                tileHeight: 128,
                tiles: tiles
