define("com/huayun/webgis/layers/support/TerrainSourceCacheWorker", [
    "./TileSource",
    "./funcUtils",
    "./TileCache",
    "./Tile",
    ...
    "../../data/ArrayType"
             ...
             utils, createVerticesFromQuantizedTerrainMesh, IndexDatatype, SegmentVector, TerrainMesh, ArrayType) {
    var water = 10;
    ...
    function TerrainSourceCache(id, options, dispatcher, width, height, url, layer, view) {
        this.id = id;
    ...
    TerrainSourceCache.prototype._tileLoaded = function _tileLoaded(tile, id, previousState, err, terrainData) {
        ...
        if (tile.workerID === undefined || tile.state === 'expired') {
            tile.workerID = this.dispatcher.send('loadTerrain', params, done.bind(this));
        } else if (tile.state === 'loading') {
            tile.reloadCallback = callback;
        } else {
            this.dispatcher.send('reloadTerrain', params, done.bind(this), tile.workerID);
        }
        var obj = this;
        function done(err, data) {
            ...
            tile.bucket = {
                layoutVertexBuffer: context.createVertexBuffer(bucket.layoutVertexArray, [
                    {name: "a_pos", type: "Float32", components: 3, offset: 0}
                ]),
                indexBuffer: context.createIndexBuffer(bucket.indexArray),
                segments: bucket.segments
            };
            tile.minimumHeight = data.minimumHeight;
            tile.maximumHeight = data.maximumHeight;
            var quadTriangleIndices = new ArrayType.StructArrayLayout3ui6();
            quadTriangleIndices.emplaceBack(0, 2, 1);
            quadTriangleIndices.emplaceBack(2, 3, 1);
            quadTriangleIndices.emplaceBack(2, 4, 3);
            quadTriangleIndices.emplaceBack(4, 5, 3);
            quadTriangleIndices.emplaceBack(4, 6, 5);
            quadTriangleIndices.emplaceBack(6, 7, 5);
            quadTriangleIndices.emplaceBack(6, 0, 7);
            quadTriangleIndices.emplaceBack(0, 1, 7);

            quadTriangleIndices.emplaceBack(1, 3, 7);
            quadTriangleIndices.emplaceBack(5, 7, 3);

            var rasterBoundsArray = new ArrayType.StructArrayLayout3i6();
            var extent = 8192;
            rasterBoundsArray.emplaceBack(0, 0, 0);
            rasterBoundsArray.emplaceBack(0, 0, 1);
            rasterBoundsArray.emplaceBack(extent, 0, 0);
            rasterBoundsArray.emplaceBack(extent, 0, 1);
            rasterBoundsArray.emplaceBack(extent, extent, 0);
            rasterBoundsArray.emplaceBack(extent, extent, 1);
            rasterBoundsArray.emplaceBack(0, extent, 0);
            rasterBoundsArray.emplaceBack(0, extent, 1);
            var segments = SegmentVector.simpleSegment(0, 0, 8, 10);

            tile.waterBucket = {
                layoutVertexBuffer: context.createVertexBuffer(rasterBoundsArray, [
                    {name: "a_pos", type: "Int16", components: 3, offset: 0}
                ]),
                indexBuffer: context.createIndexBuffer(quadTriangleIndices),
                segments: segments
            }

            tile.timeAdded = utils.now();
            obj.layer.layerView.drawHeightMap(obj.layer.layerView, tile);
            obj.layer.layerView._sourcesDirty = true;
            obj.view.threeRender();
        }

        /*
        var groundData = this.view.groundData;

        var verticesIndices = groundData.createTerrainVertice(rectangle, );
        */
        /*var canonical = tile.tileID.canonical;
        var rectangle = this.layer.tileInfo.tileXYToRectangle(canonical.x, canonical.y, canonical.z);
