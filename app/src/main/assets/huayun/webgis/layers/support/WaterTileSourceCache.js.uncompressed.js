define("com/huayun/webgis/layers/support/WaterTileSourceCache", [
    ...
    "../../utils/utils"
], function (TileSource, funcUtils, TileCache, Tile, SourceFeatureState, tileCover, Point2D, utils) {

    function WaterTileSourceCache(id, options, width, height, url, layer, view) {
        this.id = id;
        this._tiles = {};
    ...
    WaterTileSourceCache.prototype.constructor = WaterTileSourceCache;
    WaterTileSourceCache.prototype.updateTileSize = function (size) {
        this._source.setSize(size);
    };
    WaterTileSourceCache.prototype.onRemove = function onRemove(map) {
        if (this._source && this._source.onRemove) {
            this._source.onRemove(map);
        }
    };

    WaterTileSourceCache.prototype.getIds = function getIds() {
        var tileIDs = [];
    ...
    WaterTileSourceCache.prototype.getRenderableIds = function getRenderableIds() {
        var ids = [];
    ...
    WaterTileSourceCache.prototype.hasRenderableParent = function hasRenderableParent(tileID) {
    ...
    WaterTileSourceCache.prototype._isIdRenderable = function _isIdRenderable(id) {
        return this._tiles[id] && this._tiles[id].hasData() &&
            !this._coveredTiles[id] && (!this._tiles[id].holdingForFade());
    };

    WaterTileSourceCache.prototype.reload = function reload() {
        if (this._paused) {
    ...
    WaterTileSourceCache.prototype._reloadTile = function _reloadTile(id, state) {
    ...
    WaterTileSourceCache.prototype.updateTileUrl = function(url) {
        this._source.updateTileUrl(url);
    };

    WaterTileSourceCache.prototype._tileLoaded = function _tileLoaded(tile, id, previousState, err) {
        if (err) {
            tile.state = 'errored';
            if (err.status === 404) {
    ...
    WaterTileSourceCache.prototype.getTile = function getTile(tileID) {
        return this.getTileByID(tileID.key);
    };

    WaterTileSourceCache.prototype.getTileByID = function getTileByID(id) {
        return this._tiles[id];
    };

    WaterTileSourceCache.prototype.getZoom = function getZoom(transform) {
        return transform.level + transform.scaleZoom(transform.tileSize / this._source.tileSize);
    };

    WaterTileSourceCache.prototype._retainLoadedChildren = function _retainLoadedChildren(idealTiles, zoom, maxCoveringZoom, retain) {
        for (var id in this._tiles) {
            var tile = this._tiles[id];
            if (retain[id] ||
                !tile.hasData() ||
    ...
    WaterTileSourceCache.prototype.findLoadedParent = function findLoadedParent(tileID, minCoveringZoom) {
    ...
    WaterTileSourceCache.prototype.updateCacheSize = function updateCacheSize(width, height) {
    ...
    WaterTileSourceCache.prototype.update = function update(level, zoomedBounds, range, cx, cy) {
        this._coveredTiles = {};
    ...
    WaterTileSourceCache.prototype.updateTile = function updateTile(level, zoomedBounds, range, cx, cy) {
        this._coveredTiles = {};
    ...
    WaterTileSourceCache.prototype.releaseSymbolFadeTiles = function releaseSymbolFadeTiles() {
        for (var id in this._tiles) {
    ...
    WaterTileSourceCache.prototype._updateRetainedTiles = function _updateRetainedTiles(idealTileIDs, zoom) {
        var retain = {};
        var checked = {};
        var minCoveringZoom = Math.max(zoom - WaterTileSourceCache.maxOverzooming, this._source.minzoom);
        var maxCoveringZoom = Math.max(zoom + WaterTileSourceCache.maxUnderzooming, this._source.minzoom);

        var missingTiles = {};
        // countTile
        for (var i = 0, list = idealTileIDs; i < list.length; i += 1) {
            // for (var i = 6, list = idealTileIDs; i < 7; i += 1) {
            var tileID = list[i];
        ...
        for (var i$1 = 0, list$1 = idealTileIDs; i$1 < list$1.length; i$1 += 1) {
            // for (var i$1 = 6, list$1 = idealTileIDs; i$1 < 7; i$1 += 1) {
            var tileID$1 = list$1[i$1];
    ...
    WaterTileSourceCache.prototype._reAddTile = function _reAddTile(tileID) {
    ...
    WaterTileSourceCache.prototype._tileReAddLoaded = function _tileLoaded(tile, id, previousState, err) {
        this._removeTile(id);
    ...
    WaterTileSourceCache.prototype._addTile = function _addTile(tileID) {
    ...
    WaterTileSourceCache.prototype._setTileReloadTimer = function _setTileReloadTimer(id, tile) {
        var this$1 = this;
    ...
    WaterTileSourceCache.prototype._removeTile = function _removeTile(id) {
        var tile = this._tiles[id];
        if (!tile) {
            return;
        }
    ...
    WaterTileSourceCache.prototype.clearTiles = function clearTiles() {
    ...
    WaterTileSourceCache.prototype.clearOtherLevel = function clearOtherLevel(level) {
        this._shouldReloadOnResume = false;
        this._paused = false;
        for (var id in this._tiles) {
            var tile = this._tiles[id];
    ...
    WaterTileSourceCache.prototype.tilesIn = function tilesIn(pointQueryGeometry, maxPitchScaleFactor, has3DLayer) {
        var this$1 = this;
    ...
    WaterTileSourceCache.prototype.getVisibleCoordinates = function getVisibleCoordinates(symbolLayer, transform) {
        var this$1 = this;
    ...
    WaterTileSourceCache.prototype.updateTileMatrix = function updateTileMatrix(symbolLayer, transform) {
    ...
    WaterTileSourceCache.prototype.hasTransition = function hasTransition() {
    ...
    WaterTileSourceCache.maxOverzooming = 10;
    WaterTileSourceCache.maxUnderzooming = 3;
    return WaterTileSourceCache;
});
