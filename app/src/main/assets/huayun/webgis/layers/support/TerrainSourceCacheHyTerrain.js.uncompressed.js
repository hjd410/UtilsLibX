define("com/huayun/webgis/layers/support/TerrainSourceCacheHyTerrain", [
    ...
    "./TerrainMesh"
             ...
             utils, createVerticesFromQuantizedTerrainMesh, IndexDatatype, SegmentVector, ArrayType, TerrainMesh) {
    ...
    function TerrainSourceCache(id, options, dispatcher, width, height, url, layer, view) {
        ...
        this._source = new TileSource.TerrainSourceCache(id, options, dispatcher, url, view, layer);
        this._tiles = {};
        ...
        this.defaultIndexBuffer = this.view.context.createIndexBuffer(indices);
        this.defaultVertexBuffer = this.view.context.createVertexBuffer(vertices, [
            {name: "a_pos", type: "Float32", components: 3, offset: 0}
        ]);
        this.defaultSegments = SegmentVector.simpleSegment(0, 0, 4, 2);
    }
    ...
    TerrainSourceCache.prototype._tileLoaded = function _tileLoaded(tile, id, previousState, err, data) {
        if (err) {
            /*tile.state = 'errored';
            if (err.status === 404) {
                this.update(this.transform);
            ...
            return;*/
            tile.bucket = {
                layoutVertexBuffer: this.defaultVertexBuffer,
                indexBuffer: this.defaultIndexBuffer,
                segments: this.defaultSegments
            };
            tile.minimumHeight = 0;
            tile.maximumHeight = 0;
        ...
        if (data) {
            ...
            tile.bucket = {
                layoutVertexBuffer: context.createVertexBuffer(data.vertices, [
                    {name: "a_pos", type: "Float32", components: 3, offset: 0}
                ]),
                indexBuffer: context.createIndexBuffer(data.indices),
                segments: data.segments
            };
            tile.minimumHeight = data.minH;
            tile.maximumHeight = data.maxH;
            this.layer.layerView.drawHeightMap(this.layer.layerView, tile);
            this.layer.layerView._sourcesDirty = true;
            this.view.threeRender();
        } else {
            tile.bucket = {
                layoutVertexBuffer: this.layer.layerView.rasterBoundsBuffer,
                indexBuffer: this.layer.layerView.quadTriangleIndexBuffer,
                segments: this.layer.layerView.rasterBoundsSegments
            }
            tile.minimumHeight = 0;
            tile.maximumHeight = 0;
            this.layer.layerView._sourcesDirty = true;
    ...
    TerrainSourceCache.prototype.updateTile = function updateTile(level, zoomedBounds, range, cx, cy) {
        this._coveredTiles = {};
        var idealTileIDs = tileCover(level, zoomedBounds, range, "tms");
    ...
    TerrainSourceCache.prototype._updateRetainedTiles = function _updateRetainedTiles(idealTileIDs, zoom) {
        ...
        for (var i = 0, list = idealTileIDs; i < list.length; i += 1) {
            // for (var i = 0, list = idealTileIDs; i < 2; i += 1) {
            var tileID = list[i];
        ...
        for (var i$1 = 0, list$1 = idealTileIDs; i$1 < list$1.length; i$1 += 1) {
            // for (var i$1 = 0, list$1 = idealTileIDs; i$1 < 2; i$1 += 1) {
            var tileID$1 = list$1[i$1];
    ...
    TerrainSourceCache.prototype._tileReAddLoaded = function _tileLoaded(tile, id, previousState, err, data) {
        this._removeTile(id);
