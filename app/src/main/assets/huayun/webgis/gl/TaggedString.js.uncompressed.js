define("com/huayun/webgis/gl/TaggedString", [
    "../utils/Constant"
], function (Constant) {
    ...
    TaggedString.prototype.trim = function trim() {
        ...
        for (var i = 0; i < this.text.length && Constant.whitespace[this.text.charCodeAt(i)]; i++) {
            beginningWhitespace++;
        }
        var trailingWhitespace = this.text.length;
        for (var i$1 = this.text.length - 1; i$1 >= 0 && i$1 >= beginningWhitespace && Constant.whitespace[this.text.charCodeAt(i$1)]; i$1--) {
            trailingWhitespace--;
        }
