define("com/huayun/webgis/data/bucket/LineBucketSimplify2", [
    "../ArrayType",
    "../../gl/SegmentVector"
    ...
    var COS_HALF_SHARP_CORNER = Math.cos(180 / 2 * (Math.PI / 180));
    ...
    LineBucketSimplify.prototype.upload = function upload(context) {
        if (!this.uploaded) {
            this.layoutVertexBuffer = context.createVertexBuffer(this.layoutVertexArray, [
                ...
                {name: "a_data", type: "Float32", components: 4, offset: 8},
                {name: "a_normal", type: "Int16", components: 2, offset: 24}
            ]);
    ...
    LineBucketSimplify.prototype.addLine = function addLine(vertices, join, cap, miterLimit, roundLimit) {
        ...
        for (var i = first; i < len; i++) {
            ...
            */
            ...
            var isSharpCorner = false; //cosHalfAngle < COS_HALF_SHARP_CORNER && prevVertex && nextVertex;
