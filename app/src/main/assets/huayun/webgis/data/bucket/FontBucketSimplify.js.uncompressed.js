define("com/huayun/webgis/data/bucket/FontBucketSimplify", [
    "../ArrayType",
    "../../gl/SegmentVector",
    "../../geometry/Anchor",
    "../../geometry/Point2D",
    ...
    function justifyLine(positionedGlyphs, start, end, justify, lineOffset) {
        ...
        // todo 由于某些字体无法生成，暂时添加个判断。
        if (lastPositionedGlyph.metrics === null) return;
    ...
    function shapeText(text, fontFamily, glyphMap, glyphPositions, lineHeight, spacing, writingMode, translate) {
        ...
        for (var i = 0, len = textList.length; i < len; i++) {
            var aText = textList[i];
            lines[lines.length] = TaggedString.fromFeature({
                sections: [
                    {
    ...
    function getGlyphQuads(anchor, shaping, textOffset) {
        ...
        for (var i$1 = 0, list$1 = shaping.positionedLines; i$1 < list$1.length; i$1 += 1) {
            ...
            for (var i = 0, list = line.positionedGlyphs; i < list.length; i += 1) {
                ...
                // console.log(positionedGlyph.metrics.left, positionedGlyph.metrics.top);
    ...
    FontBucket.prototype.addFeature = function (geometry, text, fontFamily, glyphMap, glyphPositions, offset) {
        var lineHeight = 96;
        var shapedTextOrientations = {
            horizontal: {},
            vertical: undefined
        };
        var spacingIfAllowed = 0.96;
    ...
    FontBucket.prototype.addSymbols = function addSymbols(quads, labelAnchor) {
    ...
    FontBucket.prototype.upload = function upload(context) {
