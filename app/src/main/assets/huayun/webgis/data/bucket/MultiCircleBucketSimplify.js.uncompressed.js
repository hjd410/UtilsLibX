 ...
 *  @description : OK
 */
define("com/huayun/webgis/data/bucket/MultiCircleBucketSimplify", [
  "../ArrayType",
  "../../gl/SegmentVector"
], function (ArrayType, SegmentVector) {

  function addCircleVertex(layoutVertexArray, x, y, extrudeX, extrudeY, r, g, b, a) {
    layoutVertexArray.emplaceBack((x * 2) + ((extrudeX + 1) / 2), (y * 2) + ((extrudeY + 1) / 2), r, g, b, a);
  }
  ...
  CircleBucket.prototype.addFeature = function addFeature(geometry, colors) {
    for (var i$1 = 0, list$1 = geometry; i$1 < list$1.length; i$1 += 1) {
      var ring = list$1[i$1];
      var ringColor = colors[i$1];
      for (var i = 0, list = ring; i < list.length; i += 1) {
        var point = list[i];
        var color = ringColor[i];
        if (color.length < 4) {
          color.push(1.0);
        }
        var x = point.x;
        ...
        addCircleVertex(this.layoutVertexArray, x, y, -1, -1, color[0], color[1], color[2], color[3]);
        addCircleVertex(this.layoutVertexArray, x, y, 1, -1, color[0], color[1], color[2], color[3]);
        addCircleVertex(this.layoutVertexArray, x, y, 1, 1, color[0], color[1], color[2], color[3]);
        addCircleVertex(this.layoutVertexArray, x, y, -1, 1, color[0], color[1], color[2], color[3]);
